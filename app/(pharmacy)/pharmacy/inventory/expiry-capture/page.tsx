...
export default function ExpiryCapturePage() {
  ...
  const [priceAuthorization, setPriceAuthorization] = useState<QueueItem | null>(null)
  ...
  const capture = async (item: QueueItem, token = getCachedSpToken('price_change')) => {
    ...
    try {
      ...
      const result = await response.json()
      if (response.status === 403 && result.code === 'SP_AUTH_REQUIRED') {
      ...
      if (isSpAuthorizationRequired(error)) setPriceAuthorization(item)
      else toast.error(error instanceof Error ? error.message : 'Could not capture expiry')
    }
  ...
  <SpAuthorizationModal
    open={priceAuthorization !== null}
    action="price_change"
    description={`Authorise the imported selling price for ${priceAuthorization?.source_name || 'this item'}`}
    onAuthorized={async (token) => {
      const item = priceAuthorization
      if (item) await capture(item, token)
      setPriceAuthorization(null)
    }}
    onClose={() => setPriceAuthorization(null)}
  />
  </>
}
