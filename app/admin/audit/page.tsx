    ...
    <div className="space-y-6 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-4 gap-4">
        ...
        <div className="flex items-center space-x-2 self-start sm:self-center">
          <span className="text-xs font-semibold text-slate-500">Risk Tier:</span>
          <select
            value={filterTier}
      ...
      <div className="relative w-full max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 transition-colors"
        />
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
          <div className="flex flex-col items-center justify-center p-20">
            ...
            <span className="text-sm text-slate-500">Loading audit trail...</span>
          </div>
        ) : filteredLogs.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-20">
            <Database className="w-12 h-12 text-slate-300 mb-3" />
            <span className="text-sm font-semibold text-slate-650">No logs found</span>
            <span className="text-xs text-slate-400 mt-1">No triage events recorded in this range.</span>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-4 font-mono text-[10px] text-slate-400 truncate max-w-[150px]" title={log.query_hash}>
                      {log.query_hash}
                    </td>
